...
pub fn disable() void {
    io.out(PIC1_DATA, @as(u8,0xff));
    io.out(PIC2_DATA, @as(u8,0xff));
}

// note(shahzad): setup the pic to point interrupts outside exceptions in idt(0..0x1f)
pub fn init() void {
    const mask_1 = io.in(u8,PIC1_DATA);
    const mask_2 = io.in(u8,PIC1_DATA);
    //note(shahzad): ignoring as we only want to enable keyboard for now
    _ = .{ mask_1, mask_2 };

    //note(shahzad): send init byte
    io.out(PIC1_COMMAND, @as(u8,PIC_INIT)); //note(shahzad)!: command is only used to send eoi and init the chip
    io.wait();
    io.out(PIC2_COMMAND, @as(u8,PIC_INIT)); //note(shahzad)!: command is only used to send eoi and init the chip
    io.wait();

    //note(shahzad): send vector table offset
    io.out(PIC1_DATA, @as(u8,0x20)); //note(shahzad): 0x0..0x1f is proc exceptions so mapping it from 0x20
    io.wait();
    io.out(PIC2_DATA, @as(u8,0x28)); //note(shahzad)!: command is only used to send eoi and init the chip
    io.wait();

    //note(shahzad): set the pin conifg
    io.out(PIC1_DATA, @as(u8,MASTER_TO_SLAVE_PIN));
    io.out(PIC2_DATA, @as(u8,0x02)); //tell the pic2 chip about it's own cascading 0b10 (need to study about this shit)??
    ...
    io.out(PIC1_DATA, @as(u8,PIC_MODE_8086));
    io.out(PIC2_DATA, @as(u8,PIC_MODE_8086));

    //note(shahzad): set the interrupt mask
    io.out(PIC1_DATA, @as(u8,0));
    io.out(PIC2_DATA, @as(u8,0));
}
...
pub fn send_eoi(irq_number: u8) void {
    ...
    if (irq_number >= 8)
        io.out(PIC2_COMMAND, @as(u8,PIC_EOI));
    io.out(PIC1_COMMAND, @as(u8,PIC_EOI));
}
