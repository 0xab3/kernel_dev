#!/usr/bin/env python3
# stolen from https://github.com/Jamesits/bin2array/blob/master/bin2array.py
import binascii
import argparse

parser = argparse.ArgumentParser(
    description="Convert binary file to C-style array initializer."
)
parser.add_argument("filename", help="the file to be converted")
parser.add_argument("-O", "--output", help="write output to a file")
parser.add_argument(
    "-l", "--linebreak", type=int, help="add linebreak after every N element"
)
parser.add_argument(
    "-L",
    "--linebreak-string",
...
parser.add_argument(
    ...
    "--force-uppercase",
    action="store_true",
    help="force uppercase HEX representation",
)
parser.add_argument(
    "-n", "--newline", action="store_true", help="add a newline on file end"
)
...
def do_convension(content: bytes, to_uppercase: bool = False) -> str:
    ...
    if to_uppercase:
        hexstr = hexstr.upper()
    array = [
        args.element_prefix + hexstr[i : i + 2] + args.element_suffix
    ...
    return args.linebreak_string.join(
        [args.separator_string.join(e) + args.separator_string for e in array]
    )
